...
const FriendDetails = () => {
  ...
  <div className="mt-2">
              <button className={` px-3 py-1 rounded-full text-sm font-medium hover:bg-opacity-90 transition-all text-white ${data.status==="on-track"?"bg-[#1D3D33] ":data.status==="overdue"?"bg-red-600":"bg-amber-500"}`}>
          {data.status}
        </button>
            </div>
   ...
   <div className="flex gap-2 my-3 justify-center">
         {
            data.tags.map((tag,i)=> <span key={i} className="px-3 py-1 bg-green-100 text-green-700 text-xs font-semibold rounded-full uppercase tracking-wider">
          {tag}
          </span>)
         }
          ...
          <div className="md:col-span-2 space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              ...
              <div className="bg-white p-4 rounded-lg shadow text-center">
                <h2 className="text-2xl font-bold text-green-700">{data.days_since_contact}</h2>
            ...
            <div className="bg-white p-5 rounded-lg shadow flex justify-between items-center">
              <div>
                ...
                <p className="text-sm text-gray-500">
                  Connect every <span className="font-semibold">{data.goal} days</span>
                </p>
              </div>
